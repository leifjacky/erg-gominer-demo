...
func (m *StratumMiner) start() {
	...
	if err := m.request("mining.subscribe", []interface{}{"ergminer-v1.0.0"}); err != nil {
...
func (m *StratumMiner) handleMesg(line []byte, flag int) error {
	...
	case "mining.notify":
		...
		height, ok := params[2].(float64)
		if !ok || err != nil {
			return fmt.Errorf("invalid height")
		}
		logrus.Infof("new job: %v - %v (%d)", jobId, msg, int64(height))
		m.job.Store(&Job{
			jobId:  jobId,
			msg:    msg,
...
func (m *StratumMiner) startWorker(i int) {
	for {
		...
		hash := Autolykos2Hash(job.msg, m.nonce1, nonce2, job.height, m.target)
		bInt := Hash2BigTarget(hash)
		if bInt.Cmp(m.target) <= 0 {
			logrus.Infof("share found: %v - %064x", nonce2, bInt)
			go func() {
